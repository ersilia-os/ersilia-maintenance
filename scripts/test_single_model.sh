...
# -----------------------------------------------------------
# test_single_model.sh — test a single model by ID
#
# Usage:
#   scripts/test_single_model.sh <model_id>
#   MODEL_ID=eos4e40 scripts/test_single_model.sh
# -----------------------------------------------------------

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_INFO_FILE="${ROOT_DIR}/files/repo_info.json"
RESULTS_LOG="${ROOT_DIR}/files/single_model_test_results.txt"
...
# -----------------------------------------------------------
# Resolve the model ID from arg or env
# -----------------------------------------------------------
MODEL_ID="${1:-${MODEL_ID:-}}"
if [[ -z "${MODEL_ID}" ]]; then
  echo "ERROR: No model ID provided." >&2
  echo "Usage: $0 <model_id>" >&2
  exit 1
fi

echo "Model to test: ${MODEL_ID}"

# -----------------------------
# Helpers (same logic as test_models.sh)
# -----------------------------
...
update_json_last_test_date() {
  ...
  if [[ ! -f "${REPO_INFO_FILE}" ]]; then
    echo "WARNING: ${REPO_INFO_FILE} not found — skipping date update."
    return 0
  fi
...
update_json_last_test_outcome() {
  ...
  if [[ ! -f "${REPO_INFO_FILE}" ]]; then
    echo "WARNING: ${REPO_INFO_FILE} not found — skipping outcome update."
    return 0
  fi
...
🔎 For detailed test outputs, see: \`reports/single_model_test_summary.txt\`.
...
append_model_to_markdown() {
  ...
  echo "| ${repo_name} | ${status_symbol} | ${test_date} |" >> "${SUMMARY_MD}"
}
...
if [[ ${fetch_exit} -ne 0 || ${test_exit} -ne 0 ]]; then
  echo "ersilia fetch/test failed for ${MODEL_ID} (fetch_exit=${fetch_exit}, test_exit=${test_exit})." \
    | tee -a "${RESULTS_LOG}" | tee -a "${SUMMARY_TXT}"
  MODEL_HAS_FAILURE=true
fi
...
if [[ ! -f "${report_json}" ]]; then
  echo "WARNING: Test report JSON not found for ${MODEL_ID}: ${report_json}" \
    | tee -a "${RESULTS_LOG}" | tee -a "${SUMMARY_TXT}"
  MODEL_HAS_FAILURE=true
else
  ...
  if [[ -z "${results_json}" ]]; then
    echo "WARNING: extract.py returned empty results for ${MODEL_ID}." \
      | tee -a "${RESULTS_LOG}" | tee -a "${SUMMARY_TXT}"
    MODEL_HAS_FAILURE=true
  else
...
echo ""
echo "Test completed for ${MODEL_ID}."
